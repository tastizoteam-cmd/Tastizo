...
export async function updateAdminOfferPinStatus(req, res, next) {
    try {
        const { id } = req.params;
        if (!id || !mongoose.Types.ObjectId.isValid(id)) {
            return res.status(400).json({ success: false, message: 'Invalid offer id' });
        ...
        const updated = await adminService.updateAdminOfferPinStatus(id, isPinned);
        if (!updated) {
            return res.status(404).json({ success: false, message: 'Offer not found' });
        }
        res.status(200).json({ success: true, message: 'Offer pin status updated successfully', data: { offer: updated } });
    } catch (error) {
        next(error);
    }
}
